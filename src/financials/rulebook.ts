...
export const RULES: Rule[] = [
  ...
  { id: 'R4b', stage: 'normalize', text: 'Printed lines fused into one table row (each cell holding every line\u2019s figures), where R4c could not read them from a text layer, are split from the row itself: two lines when every cell holds exactly two figures and the second label starts a total ("Net cash ...", "Total ..."); on a scanned page, k lines (two to four) when every cell holds exactly k figures and the label has exactly k - 1 places where a new caption can start (a capitalised word not after a connecting word). Anything less certain is left unreadable.', enforced: 'splitFusedRows in normalize.ts; the split rows still pass the arithmetic checks.' },
  { id: 'R4c', stage: 'normalize', text: 'On a page with a text layer, rows the table model fused, or whose figures or captions it shifted by a line, are rebuilt as the printed lines: only when the block\u2019s figures, read down each column, are exactly those of a run of consecutive lines, and its captions and headings are those lines\u2019 words. Nothing else is rebuilt; no figure is changed, only moved to the line the page prints it on.', enforced: 'rebuildFromText in fused.ts, before R4b and R4; the rebuilt rows still pass the arithmetic checks.' },
  // --- columns and periods ------------------------------------------------------------------------

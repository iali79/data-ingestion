...
export const LABEL_RULES: Record<Statement, LabelRule[]> = {
  income: [
    rule('cost_of_sales', /^(?:less:?\s*)?cost of (?:sales|revenue|goods sold|services)\b|^direct (?:costs?|expenses)$/u),
    ...
    rule('finance_cost', /^(?:finance|financial) (?:costs?|charges)(?: - net)?$|^mark[- ]?up (?:expense|on borrowings)|^interest expense|^mark[- ]?up ?\/ ?return ?\/ ?interest expensed$/u),
    rule('interest_income', /^(?:finance|interest|mark[- ]?up) income$|^(?:profit|return|interest|mark[- ]?up) on (?:bank )?(?:deposits|balances|savings accounts?)$|^mark[- ]?up ?\/ ?return ?\/ ?interest earned$/u),
    rule('levies', /^(?:levy|levies)\b|^(?:minimum|final) tax(?:es| differential)?(?: - net)?$/u, { not: /\b(?:before|after)\b/u }),
    rule('profit_before_tax', new RegExp(String.raw`^${PROFIT} before (?:income )?tax(?:ation)?$`, 'u')),
    rule('taxation', /^(?:taxation|income tax(?: expense)?|tax(?:ation)? (?:expense|charge)|provision for taxation)(?: - net| for the (?:year|period))?(?: \(?(?:expense|charge)\)? ?\/ ?\(?credit\)?| \(?credit\)? ?\/ ?\(?(?:expense|charge)\)?)?$/u),
    ...
    rule('eps_basic', new RegExp(String.raw`^(?:(?:basic|diluted)(?: and (?:basic|diluted))? )?${PROFIT} per share\b`, 'u')),
  ],
  balance: [
    ...
    rule('share_capital', /^(?:issued,? subscribed and paid[- ]up (?:share )?capital|share capital|paid[- ]up (?:share )?capital)$/u, { side: 'claims' }),
    rule('additional_paid_in_capital', /^share premium(?: account| reserve)?$/u, { side: 'claims' }),
    rule('retained_earnings', /^(?:unappropriated profit|accumulated (?:profit|loss)(?:es)?|retained earnings|revenue reserves?)(?: \/ \(?(?:accumulated loss(?:es)?|unappropriated profit)\)?)?$|^\(?accumulated loss(?:es)?\)? \/ unappropriated profit$/u, { side: 'claims' }),
    ...
    rule('current_portion_lease_liabilities', /^current (?:portion|maturity) of (?:lease liabilit|liabilit(?:y|ies) against assets subject to finance lease)/u, { side: 'claims' }),
    rule('current_portion_long_term_debt', /^current (?:portion|maturity) of long[- ]term (?:financ|loans?|borrow)/u, { side: 'claims' }),
    rule('lease_liabilities_non_current', /^lease liabilit(?:y|ies)$|^liabilit(?:y|ies) against assets subject to finance lease$/u, { side: 'claims', under: 'non_current_liabilities' }),
    rule('long_term_debt_excl_leases', /^(?:long[- ]term (?:financ(?:e|es|ing)|loans?|borrowings?|musharaka)(?: - secured)?|term finance certificates|sukuks?|diminishing musharaka(?: - secured)?)$/u, { side: 'claims', under: 'non_current_liabilities' }),
    ...
    rule('short_term_borrowings', /^(?:short[- ]term (?:borrowings?|financ(?:e|es|ing)|loans?|running finance)|running finance(?: under mark[- ]up arrangements?)?)(?: - secured)?$/u, { side: 'claims' }),
  ...
  cash_flow: [
    ...
    rule('cf_interest_income', /^(?:interest|mark[- ]?up|profit|return) (?:income )?received(?: on (?:bank )?(?:deposits|balances|savings accounts?))?$/u, { section: 'operating' }),
    ...
    rule('sale_of_assets', /^(?:sale )?proceeds from (?:the )?(?:sale|disposal) of (?:property,? plant (?:and|&) equipment|fixed assets|operating fixed assets?)$|^sale proceeds (?:of|from) (?:(?:the )?(?:sale|disposal) of )?(?:property,? plant (?:and|&) equipment|fixed assets|operating fixed assets?)$/u, { section: 'investing' }),
